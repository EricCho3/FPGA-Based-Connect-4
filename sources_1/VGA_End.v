...
// Tool Versions: 
// Description: 
//      VGA RGB information for the ending screen
// Dependencies: 
// 
// Revision:
...
module VGA_End(
    ...
    always @(*) begin
        if (in_title_d && state != 5) begin //draws "else" conditions: scale = 2, ascii_char = " "
            scale = 3;
            case (char_counter) //we can change the case condition to shift the characters left/right since the rest would be filled by blanks (default)
                8: ascii_char = (state == 3) ? "1" : "2"; //draw is checked in if statement above, thus the other only option would be p2 win
                7: ascii_char = " ";
                6: ascii_char = "R";
                5: ascii_char = "E";
                4: ascii_char = "Y";
        ...
        else if (in_win_d && state != 5) begin
            scale = 3;
            case (char_counter) 
                6: ascii_char = "S";
                5: ascii_char = "N";
    ...
    always @(*) begin
        ...
        else if ((in_title_d || in_win_d || in_start_d) && ascii_data[7 - H_count_scaled])//(turns "player 1/2" drawing off if draw - we only want to display the word "draw")
            RGB = 12'hFFF; //white
        else    
            RGB = 12'hF00; //red
    end
